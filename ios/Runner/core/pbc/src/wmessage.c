...
int pbc_wmessage_uint32(struct pbc_wmessage *m, const char *key, uint32_t n) {
	return pbc_wmessage_integer(m,key,n,0);
}

int pbc_wmessage_uint64(struct pbc_wmessage *m, const char *key, uint64_t n) {
	return pbc_wmessage_integer(m,key,(uint32_t)n,(uint32_t)(n>>32));
}
