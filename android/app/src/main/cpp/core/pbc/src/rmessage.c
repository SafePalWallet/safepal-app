...
pbc_rmessage_string(struct pbc_rmessage * m , const char *key , int index, int *sz) {
	...
	return var->s.str?var->s.str:"";
}

const char * pbc_read_string(struct pbc_rmessage *m , const char *key){
	return pbc_rmessage_string(m,key,0,NULL);
}

const char * pbc_read_bytes(struct pbc_rmessage *m , const char *key ,int *sz){
	return pbc_rmessage_string(m,key,0,sz);
}
...
uint32_t pbc_rmessage_uint32(struct pbc_rmessage *m , const char *key, int index){
	return pbc_rmessage_integer(m,key,index,NULL);
}

uint64_t pbc_rmessage_uint64(struct pbc_rmessage *m , const char *key , int index){
	uint32_t h=0;
	uint32_t l = pbc_rmessage_integer(m,key,index,&h);
	return ((uint64_t)h) << 32 | (uint64_t)l;
}

uint32_t pbc_read_uint32(struct pbc_rmessage *m , const char *key) {
	return pbc_rmessage_uint32(m,key,0);
}

uint64_t pbc_read_uint64(struct pbc_rmessage *m , const char *key) {
	return pbc_rmessage_uint64(m,key,0);
}
...
struct pbc_rmessage * pbc_read_message(struct pbc_rmessage * rm, const char *key){
	return pbc_rmessage_message(rm,key,0);
}
