...
static const char *QRCODE_PREFIX_TXT_BIN = "A:";
...
int is_bin_qr_packet(const char *data, size_t size) {
    if (size > QRCODE_PREFIX_LEN && data[1] == ':') {
        ...
        if (*data == QRCODE_PREFIX_TXT_BIN[0]) {
            return 2;
        }
    }
    return 0;
...
int decode_qr_packet(const char *data, size_t size, qr_packet *packet) {
    ...
    int bin_len;
    if (isbase64) {
        bin_len = Base64decode_len(data + QRCODE_PREFIX_LEN, size);
        if (bin_len <= 0) {
            db_error("get decode len false");
            return -1;
        }
        rawdata = (unsigned char *) malloc(bin_len);
        if (rawdata == NULL) {
            db_error("malloc false");
            return -1;
        }
        bin_len = Base64decode(rawdata, data + QRCODE_PREFIX_LEN, size);
    } else {
...
static unsigned char *encode_qr_packet(const unsigned char *data, size_t size, int qrtype, int msg_type, int flag,
    ...
    *p++ = (unsigned char) totallen;
    ...
    if (qrtype == QR_TYPE_B64) {
        packetLen = QRCODE_PREFIX_LEN + Base64encode_len(totallen);
        unsigned char *b64_rawdata = (unsigned char *) malloc(packetLen);
        if (!b64_rawdata) {
            free(rawdata);
            db_error("malloc memory size:%d false,", packetLen);
            return NULL;
        }
        memcpy(b64_rawdata, QRCODE_PREFIX_TXT_BIN, QRCODE_PREFIX_LEN);
        Base64encode((char *) (b64_rawdata + QRCODE_PREFIX_LEN), rawdata, totallen, 0);
        free(rawdata);
        rawdata = b64_rawdata;
    }
    if (packet_size != NULL) {
...
int split_qr_packet(qr_packet_chunk_info *chunk_result, const unsigned char *qrdata, int size, int qrtype, int msg_type, int flag,
    ...
    if (qrtype == QR_TYPE_B64) {
        max_chunk_size = max_chunk_size / 4 * 3;
    }
