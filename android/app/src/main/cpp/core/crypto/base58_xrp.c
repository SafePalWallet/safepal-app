...
static int b58check(const void *bin, size_t binsz, HasherType hasher_type,
...
int base58_xrp_encode_check(const uint8_t *data, int datalen,
...
int base58_xrp_decode_check(const char *str, HasherType hasher_type, uint8_t *data,
                            int datalen) {
	if (datalen > 128) {
		return 0;
	}
