...
class QRPlugin extends BasePlugin {
  static const MethodChannel _channel =
      const MethodChannel('flutter.safepal.io/qr');
  ...
  static Future<List<Uint8List>> getQRImageData({
    ...
    paras['msg_type'] =
        (msgType == null) ? MessageType.MSG_UNKOWN.value : msgType;
    ...
    List<dynamic>? result =
        await invokeMethod<List<dynamic>>('split_qr_data', paras);
    return List.from(result!);
  }

  static Future<dynamic> launchQRCodeScan(BuildContext context,
      ...
      QRScanRespExtHeaderHandler? extHeaderHandler}) async {
    Map args = {};
    ...
    args['no_photo_permission'] =
        "The access to Album is disabled. Please approve the access first. ";
    args['ok'] = "OK";

    if (Platform.isIOS) {
      args["iosCameraPermissionsTips"] =
          "Allow Safepal to access your camera and microphone on Settings-privacy on your iPhone";
    }

    Map<dynamic, dynamic>? qrResp =
        await invokeMethod<Map<dynamic, dynamic>>('show_qr_scan', args);
    if (qrResp == null) {
      return null;
    }
    if (qrResp["cancel"] == 1) {
    ...
    if (qrResp['message_type'] != null) {
      ...
      if (errorCode != null && errorCode != 0) {
        Alert.show(
            context: context,
            content: "System Error(code=$errorCode)",
            options: ["OK"],
            onPress: (idx) {});
        return null;
      }
      ...
      if (data != null &&
          (data is String) &&
          messageType == MessageType.MSG_UNKOWN) {
        string = data;
      } else if (data != null &&
          (data is Uint8List || data is List<int>) &&
          messageType == MessageType.MSG_UNKOWN) {
